    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult ChangePassword(ChangePasswordModel model)
        {
            if (!ModelState.IsValid)
            {
                ...
                return new BadResponse(sbText.ToString());
            }


            var result = UserManager.ChangePassword(User.Identity.GetUserId(), model.CurrentPassword, model.Password);
            if (result.Succeeded)
            {
                return new OkResponse();
            }

            return new BadResponse(result.Errors.ElementAt(0));
        }
